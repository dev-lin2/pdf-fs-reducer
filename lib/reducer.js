...
function validateOptions({ input, preset, dpi, compatibility }) {
  if (!input) {
...
async function reducePDF({
  ...
  validateOptions({ input, preset, dpi, compatibility });
  ...
  const modeLabel =
    dpi !== undefined && dpi !== null
      ? `Custom DPI (${dpi})`
      : (PRESETS[preset] && PRESETS[preset].label) || `Preset (${preset})`;
